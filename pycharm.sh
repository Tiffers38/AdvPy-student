...
if [ -n "$PYCHARM_JDK" -a -x "$PYCHARM_JDK/bin/java" ]; then
  JDK="$PYCHARM_JDK"
elif [ -s "$HOME/.PyCharmEdu30/config/pycharm.jdk" ]; then
  JDK=`$CAT $HOME/.PyCharmEdu30/config/pycharm.jdk`
  if [ ! -d $JDK ]; then
    JDK=$IDE_HOME/$JDK
  fi
...
elif [ -r "$HOME/.PyCharmEdu30/pycharm$BITS.vmoptions" ]; then
  # user-overridden
  VM_OPTIONS_FILE="$HOME/.PyCharmEdu30/pycharm$BITS.vmoptions"
...
if [ "$IS_EAP" = "true" ]; then
  ...
  if [ -r "$IDE_BIN_HOME/lib$AGENT_LIB.so" ]; then
    AGENT="-agentlib:$AGENT_LIB=disablealloc,delay=10000,sessionname=PyCharmEdu30"
  fi
fi
...
LD_LIBRARY_PATH="$IDE_BIN_HOME:$LD_LIBRARY_PATH" "$JAVA_BIN" \
  ...
  -Djb.restart.code=88 -Didea.paths.selector=PyCharmEdu30 \
...
RESTARTER="$HOME/.PyCharmEdu30/system/restart/restarter.sh"
if [ -x "$RESTARTER" ]; then
  "$RESTARTER"
  "$RM" -f "$RESTARTER"
fi
